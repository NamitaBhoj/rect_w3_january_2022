...
class Offices extends React.Component {
                ...
                if (response.ok) {
                    ...
                    response.json().then(json_response => {
                        ...
                        this.setState({
                            ...
                            current_office: json_response.offices.length
                                ? json_response.offices[0]
                                : null,
    ...
    changeOfficeIndex = action => {
        ...
        } else newIndex = this.state.offices_index + 1
        this.setState({
            offices_index: newIndex,
            current_office: this.state.offices_data[newIndex]
        })
    }
    ...
    addOrUpdateOffice = () => {
        if (this.state.current_office) {
            ...
            // update
            console.log('update')
            const id = this.state.offices_data[this.state.offices_index]
                .officecode
            fetch('http://localhost:8000/offices/' + id, {
                method: 'PUT',
                headers: {
        ...
        return (
            <form className={styles.officeForm}>
                <div>
                    ...
                    <input
                        value={office ? office.officecode ?? '' : ''}
                        type='number'
                ...
                <div>
                    ...
                    <input
                        value={office ? office.addressline1 ?? '' : ''}
                        type='text'
                        onChange={event => this.onChangeFormField(event)}
                        id='address_line_1'
                        name='addressline1'
                ...
                <div>
                    ...
                    <input
                        value={office ? office.addressline2 ?? '' : ''}
                        type='text'
                        onChange={event => this.onChangeFormField(event)}
                        id='address_line_2'
                        name='addressline2'
                ...
                <div>
                    ...
                    <input
                        value={office ? office.postalcode ?? '' : ''}
                        type='number'
                        onChange={event => this.onChangeFormField(event)}
                        id='postal_code'
                        name='postalcode'
                ...
                <div className='text-center'>
                    <button
                        type='button'
                        onClick={() => this.addOrUpdateOffice()}
                    >
                        Save{' '}
                    </button>
                    <button
                        className={this.state.current_office ? '' : 'd-none'}
                        type='button'
                    ...
                    <button type='button' onClick={() => this.clearForm()}>
                        Clear form to Add a new Office{' '}
                    </button>
                </div>
                <br />
            </form>
            ...
            if (this.state.offices_count !== 0) {
                return (
                    <div className={styles.offices}>
                        ...
                        {this.officeHTML(
                            this.state.current_office
                                ? this.state.current_office
                                : this.state.new_office
                        )}
